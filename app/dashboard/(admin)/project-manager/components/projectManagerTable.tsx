...
function ActionButtons({
  ...
  return (
    <div className={`flex gap-2 ${compact ? "items-center justify-center" : ""}`}>
      <button
        onClick={onEdit}
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-100 dark:border-zinc-800 p-4 flex flex-col gap-3">
      <div className="flex flex-col gap-1 min-w-0">
        <span className="text-[10px] text-zinc-300 dark:text-zinc-700 tabular-nums">#{index}</span>
        <p className="text-[13px] font-medium text-zinc-800 dark:text-zinc-100 truncate">{item.name}</p>
        <p className="text-[11px] font-mono text-zinc-400 dark:text-zinc-500">@{item.username}</p>
      </div>

      <p className="text-[12px] text-zinc-500 dark:text-zinc-400 truncate">{item.email}</p>

      <p className="text-[11px] text-zinc-300 dark:text-zinc-700">{formatDate(item.createdAt)}</p>
...
function EmptyState({ colSpan }: { colSpan?: number }) {
  const text = <p className="text-[13px] text-zinc-300 dark:text-zinc-700">Belum ada project manager.</p>;
  if (colSpan) {
    return (
      <tr>
        <td colSpan={colSpan} className="py-16 text-center">{text}</td>
      </tr>
    );
  }
...
export default function ProjectManagerTable({
  ...
  const [deleteTarget, setDeleteTarget] = useState<ProjectManagerData | null>(null);
    ...
    >
      ...
      {deleteTarget && (
        <DeleteProjectManagerModal
          ...
          onSaved={() => { setDeleteTarget(null); refresh(); }}
        />
      )}
              ...
              {initialManagers.length === 0 ? (
                ...
                initialManagers.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-[12px] text-zinc-500 dark:text-zinc-400">
                      {item.email}
                    </td>

                    <td className="py-4 px-4 text-[12px] font-mono text-zinc-400 dark:text-zinc-500">
                      @{item.username}
                    </td>
